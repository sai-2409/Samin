...
function onYaPayLoad() {
  ...
  // Load merchant ID from backend
  async function getMerchantId() {
    try {
      const response = await fetch("/api/get-api-keys");
      const keys = await response.json();
      return keys.yandex_merchant_id;
    } catch (error) {
      console.warn("Failed to load merchant ID:", error);
      return null;
    }
  }
    ...
    YaPay.createSession(paymentData, {
      ...
      .then(function (paymentSession) {
        paymentSession.mountButton(
          document.querySelector("#yandex-pay-button"),
          {
  ...
  // Call initializePayment to start the process
  initializePayment();
}
